...
Touker 移动端页面存在多层内部滚动容器和懒加载列表。采集器必须滚动所有可滚动容器，并记录滚动脚本返回的 `moved`、`loading`、`itemCount` / `codeCount` 等信号；不能把滚动脚本缺失 `moved` 字段误判为已经到底。当页面仍出现“加载中”，或识别代码数少于 `expected_count` 时，必须继续滚动或按硬阻断停止，不能生成分析和报告。
