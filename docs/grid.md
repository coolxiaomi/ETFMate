...
5. 可执行网格必须是双边计划：买入侧和卖出侧都应有明确的 100 股整数倍数量。单边卖出不是网格，只能输出为减仓、清仓或暂停类执行策略。
...
9. 输出完整结构给报告和 AI 复核输入；单边卖出输出 `execution_plan_type`，不计入网格
```

## 4. 适用性规则

只有满足以下任一条件时，才进入网格/执行策略评估：

1. 已有 Touker 网格。
2. 已有当前持仓。
3. 未持仓自选 ETF 的规则动作允许建仓或轻仓建仓。

最终是否计入“可执行网格”还必须通过双边数量闸门：买入侧和卖出侧都为 `ACTIVE`，且建议数量都不低于 100 股并为 100 股整数倍。未持仓建仓、当前无可卖一手、买入侧停用或卖出侧不足一手时，不计入网格，应展示为执行策略或暂不设网格。
...
`execution_plan_*` 只在 `grid_applicable=false` 且需要清仓、减仓或暂停时出现。报告将双边网格建议嵌入每只 ETF 的主表；执行策略只展示策略说明、卖出计划和原 Touker 网格处理建议，不渲染完整买/卖网格表。AI 复核输入会读取同一套字段，不允许报告和 AI 使用不同的网格契约。
...
主动作必须互斥。严重风险优先于调宽/调窄；触发暂停买入侧、弱势减仓或只卖清仓时，不再把它称为网格间距调参，而应切换为执行策略。
...
| WEAK_REDUCE | 弱势减仓 | 输出 `REDUCE_PLAN` 执行策略，不属于网格 |
| ONLY_SELL_OR_CLEAR | 只卖清仓 | 输出 `CLEAR_PLAN` 执行策略，不属于网格 |
| PAUSE | 暂停 | 输出 `PAUSE_PLAN` 或暂不设网格，不属于网格 |
...
用途判断要区分盈利延续和风险保护。盈利且趋势评分高、风险等级低、MA 未转弱时，维持为持仓网格并保留盈利空间；趋势转弱、风控复核、目标仓位为 0 或只保留卖出时，改为清仓/退出策略或反弹减仓策略，不再称为网格。
...
1. 若趋势评分不低于 70、风险等级 LOW，且价格未低于 MA60/MA20，维持现有基准，保留继续盈利空间。
...
维护边界：ATR 只用于网格间距、基准价偏离容忍和条件单参数，不得回流修改 `ShortTrendScore`、趋势分层或仓位动作边界。RSI/BIAS 过热风险由趋势评分和规则引擎输出，再由网格层消费。
...
未持仓建仓默认 100，但如果没有可执行卖出侧，不计入双边网格
```
...
2. 卖出数量不得超过当前持仓数量。
3. 趋势交易模式下，`WEAK_REDUCE` / `ONLY_SELL_OR_CLEAR` / `PAUSE` 必须输出 `grid_applicable=false` 和对应 `execution_plan_type`；同时用 `buy_execution_status=DISABLED`、`suggested_buy_quantity=null` 表示停用买入侧。报告不得把停用侧渲染成非数字买股数或零股条件单，而应改成“买入侧停用/删除买入条件单”的执行说明。
...
7. 高位保护但仍保留双边网格时，买入侧必须给出真实 100 股整数倍数量；若系统判断不应买入，则应切换为停用买入侧的策略展示，不得留下空买量。
...
如果只剩卖出侧纪律，不能通过增大买入间距、隐藏买入数量或把买入数量写成停用文案来伪装成网格。正确输出是 `CLEAR_PLAN` 或 `REDUCE_PLAN`，并建议先暂停或删除原 Touker 双边网格，再按卖出计划执行。

### 11.3 风控复核与退出短线仓

当持仓动作是 `RISK_REVIEW`、`EXIT_SHORT_TERM`，或规则动作是“风控复核 / 退出短线仓位”，或趋势评分低于 40：
...
1. 只有双边网格建议才嵌入完整网格表。
2. `grid_applicable=false` 且无 `execution_plan_type` 时，只显示暂不设网格原因。
3. 报告展示优先使用 `grid_mode_label`，弱趋势不得再把“只卖清仓/弱势减仓”压缩成“降低买”。
4. 报告展示的买卖数量必须来自执行校验后的结果；卖出不得超过当前持仓。
5. `ONLY_SELL_OR_CLEAR` 不展示为完整买卖网格，而展示为清仓/退出策略；`WEAK_REDUCE` 展示为反弹减仓策略。两者都不展示买入条件单行，不计入网格数量，不进入“网格”分组；买入侧通过“停用/删除买入条件单”说明表达，避免出现非数字买股数。
6. `PROFIT_PROTECTION` 仍属于可执行止盈网格；如果买入侧未停用，建议买股数必须是 100 股整数倍，不能留空。

## 15. 回归测试

修改网格建议时至少关注 `tests/test_decision_consistency.py` 中这些场景：

1. 高风险目标仓位为 0 时，不能输出网格，必须输出清仓/退出执行策略。
...
7. 弱势浮亏且仓位不低时，不能输出单边网格，必须输出反弹减仓执行策略。
8. 未持仓自选 ETF 即使规则允许建仓/轻仓建仓，也必须通过双边数量闸门；没有可执行卖出侧时不得生成建仓网格。
