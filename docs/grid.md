...
证据置信度不足时，网格调参必须降强度。当前实现中置信度低于 45 时，不做过细调宽/调窄；多层证据偏弱时，买入侧继续降档。
...
2. 卖出数量不得超过当前持仓数量。
3. 不用“买入数量 0”表达停买。高风险停买用“暂停买入侧 / 只保留卖出 / 人工复核”表达，并提示需要人工停用买触发。
...
如果护栏包含证据不足、风险未确认或仓位偏高，买入侧需要再降一档。若持仓已有不低浮盈且趋势/风险确认不足，卖出侧保持更积极。
...
3. 报告展示可简写“降低买入侧”为“降低买”，但底层动作字段保持完整中文。
4. “停买”不能表现为数量 0 的设置项，只能作为人工风险提示。
...
6. 弱势浮亏且仓位不低时，买入侧必须降速。
7. 未持仓自选 ETF 只有在规则允许建仓/轻仓建仓时才生成建仓网格。
8. 未持仓且规则为观察时，必须输出“暂不设网格”。
