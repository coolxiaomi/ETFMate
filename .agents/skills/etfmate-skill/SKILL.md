...
- 同花顺采集必须覆盖当前持仓、可用数量、成本价、现价、市值、浮盈亏、仓位占比、交易记录、清仓数据、持仓备注/看法列，以及自选 ETF 池。持仓、已清仓、交易记录和自选 ETF 池都是滚动加载列表，不能只取首屏或默认 DOM；必须在滚动过程中累积每一屏的 DOM/table/text 快照，直到滚动到底且数据稳定。自选 ETF 池以 `https://tzzb.10jqka.com.cn/pc/index.html#/myAccount/a/ISUeEwK` 为主来源；自选页无结果时才允许退回持仓页缓存/DOM 兜底。自选池需要保存纳入列表、过滤列表和过滤原因，保留场内基金代码段（如 15/16/50/51/52/56/58 开头）及名称包含 ETF/LOF/基金/REIT 的合规标的，过滤可转债、A 股股票、港股股票和非 ETF 标的。
...
- 快速导航必须是顶部一眼可扫的决策区：每只 ETF 显示短动作（如加仓、建仓、减仓、持有、等待、暂停买入）和颜色提示，并在 ETF 前增加“热/温/平/凉/寒”温度图标或等价 icon。温度含义按动作和风险归类：热偏加仓/建仓，温偏观察/等待，平偏持有，凉偏减仓/降低，寒偏暂停/禁止/高风险。
