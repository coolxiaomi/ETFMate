...
- 网格条件单委托股数必须按 A 股最小一手处理：最小 100 股，且只能是 100 的整数倍。除“暂停买入侧”允许买入数量为 0 外，买入/卖出建议数量都必须钳制到 100 股倍数。

## HTML 报告要求

- 标题使用“实时 ETF 持仓与网格分析”，显示具体分析时间。
- 页面提供 ETF 快速导航，点击代码/名称可跳转到对应 ETF 小节。
- 顶部数量汇总按“持仓、网格、ETF池、全部”显示；ETF池只统计同花顺投资账本自选页纳入的 ETF/LOF/场内基金，不得把持仓并入 ETF池；“全部”才是持仓、网格、自选池去重后的全集。
- 顶部数量汇总必须是可点击筛选按钮：点击持仓只显示持仓 ETF，点击网格只显示有 Touker 网格的 ETF，点击 ETF池只显示自选池 ETF，点击全部显示全集；快速导航区和 ETF 卡片必须同步跟随筛选变化。
- 快速导航必须是顶部一眼可扫的决策区：每只 ETF 显示短动作（如加仓、建仓、减仓、持有、等待、暂停买入）和颜色提示，并在 ETF 前增加“热/温/平/凉/寒”温度图标或等价 icon。温度只表达趋势温度，不表达动作推荐：热=趋势评分≥85，温=75-84，平=60-74，凉=45-59，寒=<45；动作倾向由短动作文字和决策分组表达。
...
- 报告中方向文案要简洁：`暂停网格` 展示为“停买”，`降低买入侧` 展示为“降低买”，`买入侧/卖出侧` 在报告展示中尽量简化为“买/卖”。“停买”不是关闭整个 Touker 条件单，而是买入数量降为 0 或临时停用买触发，卖出/止盈纪律保留。
