...
def daily(amount):
    return pd.DataFrame({
        "datetime": pd.bdate_range(end="2026-09-07", periods=80).strftime("%Y-%m-%d"),
...
def test_snapshot_uses_real_historical_amount(monkeypatch):
    monkeypatch.setattr(providers, "tencent_quote", lambda codes: {codes[0]: {"last_price": 1.0, "amount": 999, "quote_time": "2026-09-08T10:38:54+08:00"}})
    monkeypatch.setattr(providers, "baidu_daily_kline", lambda code: daily([100_000_000.0] * 80))
    monkeypatch.setattr(providers, "tencent_daily_kline", lambda code: daily([float("nan")] * 80).assign(volume=10.0))
    result = providers.build_market_snapshot("510500")
    assert result.amount_avg20 == 100_000_000.0
    assert result.amount == 999
    assert "kline:tencent_qfq+baidu_amount" in result.data_quality
